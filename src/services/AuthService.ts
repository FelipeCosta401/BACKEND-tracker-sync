...
export default class AuthService {
  ...
  async register(credentials: registerCredentialsInterface, creatorUserRole: userRole) {
    const emailAlreadyExists = await this.userExists(credentials.email);
    if (emailAlreadyExists) {
      throw new AppError("Email já cadastrado!");
    }

    const registryNumberExists = await this.userExists(
      String(credentials.registryNumber)
    );
    if (registryNumberExists) {
      throw new AppError("Número de registro já cadastrado!");
    }

    let acceptableNewUserRole: userRole[] = ["ADMIN", "AGENT", "LEADER", "SUPPORT"]
    if(creatorUserRole === "LEADER"){
      acceptableNewUserRole = acceptableNewUserRole.filter((role) => role !== "ADMIN") 
    }
    if(!acceptableNewUserRole.includes(credentials.role)) {
      throw new AppError("Usuário não tem permissão para cadastrar um registro com esse nível de acesso!", 403)
    }
